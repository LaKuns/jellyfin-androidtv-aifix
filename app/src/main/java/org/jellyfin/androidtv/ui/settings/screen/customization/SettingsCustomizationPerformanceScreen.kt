...
@Composable
fun SettingsCustomizationPerformanceScreen() {
	...
	SettingsColumn {
		item {
			ListSection(
				...
				captionContent = { Text(stringResource(R.string.performance_mode_description)) },
			)
		}

		items(PerformanceMode.entries) { entry ->
			ListButton(
				headingContent = { Text(stringResource(entry.nameRes)) },
				trailingContent = { RadioButton(checked = performanceMode == entry) },
				onClick = {
					performanceMode = entry
					router.back()
				},
				modifier = Modifier.focusKey("performance_mode_${entry.name}", initialFocus = performanceMode == entry),
			)
		}
	}
}
